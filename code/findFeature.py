    ...
    def findFeature(self):
        ...
        for c in self.cube:
            ...
            for feature in self.featues:
                if peak_feature[0] == 1 and feature[0] == 10:
...
class GetInfo():
    ...
    def find_window_info(self,windows):
        ...
        #get ab
        info_ab = self.getDetailInfo(win1["position"],index_a,index_b)
        #get bc
        info_bc = self.getDetailInfo2(win1["position"],win2["position"],index_b,index_c)
        #get de
        info_de = self.getDetailInfo(win2["position"],index_d,index_e)
        #get cd
        info_cd = None
        if win2["position"] == 1:
            ...
            ave = i / time
            info_cd = {"pos": pos, "max": max_value, "integrate": i, "average": ave, "timeLong": time}
        return [info_ab,info_bc,info_cd,info_de]
    def find_cube_info(self,Cube):
        result = []
        for windows in Cube:
            win_info = self.find_window_info(windows)
...
for c in reminder_cube:
    info_cube = info.find_cube_info(c)
    print(info_cube)
    saver = ExcelSaver3()
    saver.write_data(info_cube)
    saver.save(filename="cube"+str(nums),dirname=filename)
    nums += 1
